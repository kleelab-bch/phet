...
# load donors data
donors = pd.read_csv(os.path.join(RESULT_PATH, "plasschaert_mouse", "plasschaert_mouse_groups.csv"),
                     sep=',', index_col=0, header=None)
...
markers_dict = temp_dict
markers = [f for k, item in markers_dict.items() for f in item]
basal_markers = ["KRT5", "TRP63", "KRT14", "KRT15", "KRT19", "PDPN", "NGFR",
                 "LGALS1", "ITGA6", "ITGB4", "LAMA3", "LAMB3", "S100A2",
                 "NPPC", "BCAM", "DST", "KRT19", "NOTCH3", "DCN", "COL17A1",
                 "ABI3BP"]
basal_markers += list(set([f.upper() for item in basal_subsets_markers.values()
                           for f in item]))
secretory_markers = ["SCGB1A1", "MUC5AC", "MUC5B", "TFF3", "SCGB3A1", "SCGB3A2",
                     "BPIFB1", "MSMB", "SLPI", "WFDC2", "BPIFA1", "MSLN", "AGR2"]
selected_features_dict = {'Basal': ['KRT5', 'TRP63', 'LAMB3',
                                    'KRT15', 'DCN', 'COL17A1'],
                          'Secretory': ['MUC5B', 'SCGB1A1', 'SCGB3A2',
                                        'BPIFA1', 'MSLN', 'AGR2']}
selected_features_dict = {'Basal': basal_markers,
...
# load trajectory data
top_cluster = pd.read_csv(os.path.join(RESULT_PATH, "plasschaert_mouse",
                                       "plasschaert_mouse_basal_top.txt"),
                          sep=',', index_col=0, header=None)
top_cluster = np.squeeze(top_cluster.values.tolist())
bottom_cluster = pd.read_csv(os.path.join(RESULT_PATH, "plasschaert_mouse",
                                          "plasschaert_mouse_basal_bottom.txt"),
                             sep=',', index_col=0, header=None)
bottom_cluster = np.squeeze(bottom_cluster.values.tolist())
samples_idx = np.append(top_cluster, bottom_cluster)
samples_name = ["Top cluster"] * len(top_cluster) + ["Bottom cluster"] * len(bottom_cluster)
...
selected_features_dict = {'Basal': ['KRT5', 'TRP63'],
                          'KRT4/13+': ['KRT4', 'KRT13'],
                          'Secretory': ['MUC5B', 'SCGB1A1', 'NOTCH2', 'BPIFA1',
                                        'MSLN', 'AGR2']}
temp_dict = {}
for key, items in selected_features_dict.items():
    temp = []
...
bottom_cluster = pd.read_csv(os.path.join(RESULT_PATH, "plasschaert_mouse",
                             ...
                             sep=',', index_col=0, header=None)
bottom_cluster = np.squeeze(bottom_cluster.values.tolist())
bottom_trans_cluster = pd.read_csv(os.path.join(RESULT_PATH, "plasschaert_mouse",
                                                "plasschaert_mouse_basal_bottom_transition.txt"),
                                   sep=',', index_col=0, header=None)
bottom_trans_cluster = np.squeeze(bottom_trans_cluster.values.tolist())

samples_idx = np.append(bottom_cluster, bottom_trans_cluster)
samples_name = ["Bottom cluster"] * len(bottom_cluster) + ["Bottom Trans cluster"] * len(bottom_trans_cluster)
samples_name = pd.Series(samples_name)
...
with plt.rc_context({'figure.figsize': (8, 6), 'axes.titlesize': '24'}):
    sc.pl.umap(adata, color=['clusters'],
               use_raw=False, add_outline=False, legend_loc='on data',
               legend_fontsize=40, legend_fontoutline=2, frameon=False,
               palette={"Bottom cluster": "black", "Bottom Trans cluster": "red"})
# Filter markers
temp_dict = {}
for key, items in markers_dict.items():
    temp = []
...
bottom_trans_cluster = pd.read_csv(os.path.join(RESULT_PATH, "plasschaert_mouse",
                                   ...
                                   sep=',', index_col=0, header=None)
bottom_trans_cluster = np.squeeze(bottom_trans_cluster.values.tolist())
samples_idx = np.append(top_cluster, bottom_trans_cluster)
samples_name = ["Top cluster"] * len(top_cluster) + ["Bottom Trans cluster"] * len(bottom_trans_cluster)
samples_name = pd.Series(samples_name, dtype="category")
...
with plt.rc_context({'figure.figsize': (8, 6), 'axes.titlesize': '24'}):
    sc.pl.umap(adata, color=['clusters'],
               use_raw=False, add_outline=False, legend_loc='on data',
               legend_fontsize=40, legend_fontoutline=2, frameon=False,
               palette={"Top cluster": "black", "Bottom Trans cluster": "red"})
# Filter markers
temp_dict = {}
for key, items in markers_dict.items():
    temp = []
