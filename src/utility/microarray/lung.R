...
gset <- read.table(file.path(working_dir, paste(file_name, ".tsv", sep = "")),
...
gsms <- c(0, 1, 1, 1, 1)
names(gsms) <- c("NL ", "AD ", "SMCL ", "SQ ", "COID ")
gsms <- gsms[classes]
...
par(mar = c(4, 4, 2, 1))
title <- paste("Lung value distribution", sep = "")
...
plotSA(fit2, main = "Mean variance trend, Lung")
