...
file_name <- "plasschaert_human_basal2secretory_vs_ionocytes"
...
# keep Basal cells 
condition <- metadata %in% c("Basal>Secretory", "Ionocytes")
...
enriched_features <- df$EnrichedIn %in% c("Basal>Secretory", "Ionocytes")
...
rownames(tT) <- NULL
tT <- cbind("ID" = temp, tT)
write.table(tT, file = file.path(working_dir, paste(file_name, "diff_features.csv",
                                                    sep = "")),
            sep = ",", quote = FALSE, row.names = FALSE)
