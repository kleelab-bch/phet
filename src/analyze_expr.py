...
def train(num_jobs: int = 4):
    ...
    df = pd.DataFrame(list_scores, columns=["F1 scores"], index=methods_name)
    ...
    plot_barplot(X=list_scores, methods_name=methods_name, metric="f1", file_name=file_name,
    ...
    score = plot_umap(X=X, y=y, subtypes=subtypes, features_name=features_name, num_features=num_features,
                      ...
                      num_clusters=num_clusters, max_clusters=max_clusters, heatmap_plot=False, num_jobs=num_jobs,
                      suptitle=None, file_name=file_name + "_all", save_path=RESULT_PATH)
    list_scores.append(score)
    ...
    for method_idx, item in enumerate(methods_dict.items()):
        ...
        score = plot_umap(X=X[:, temp], y=y, subtypes=subtypes, features_name=temp_feature, num_features=num_features,
                          ...
                          heatmap_plot=False, num_jobs=num_jobs, suptitle=method_name.upper(),
                          file_name=file_name + "_" + method_name.lower(), save_path=RESULT_PATH)
        list_scores.append(score)

    df = pd.DataFrame(list_scores, columns=["Accuracy scores"], index=["All"] + methods_name)
    df.to_csv(path_or_buf=os.path.join(RESULT_PATH, file_name + "_cluster_accuracy.csv"), sep=",")

    print("## Plot barplot using to demonstrate clustering accuracy...".format(topKfeatures))
    plot_barplot(X=list_scores, methods_name=["All"] + methods_name, metric="accuracy",
